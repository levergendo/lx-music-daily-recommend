...
async function generateDailyRecommend(forceRefresh = false) {
  try {
    ...
    const seedSongs = await getSeedSongs();
    ...
    const similarResults = await Promise.all(
      seedSongs.filter(s => s.id).map(song => getSimilarSongs(song.id, 10))
    );
    const allSimilarSongs = similarResults.flat();
    ...
    const filteredSongs = filterSongs(allSimilarSongs, userSettings);
    ...
    const result = uniqueSongs.slice(0, dailyCount);
    ...
    if (typeof window !== 'undefined' && localStorage) {
      ...
      localStorage.setItem('recent_discover_playlist', JSON.stringify(result));
      setGeneratedToday();
    }

    showMessage(`推荐生成成功，共 ${result.length} 首歌曲`, 'success');
    return result;
  } catch (error) {
